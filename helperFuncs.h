...
double getRandomDouble(const double low, const double high)
{
    using namespace std;
    random_device rd;
    mt19937 gen(rd());
...
Matrix2d matrixMultiply(const Matrix2d& x, const Matrix2d &y)
{
...
Matrix1d matrixAdd(const Matrix1d& x, const Matrix1d& y)
{
...
void printMatrix2D(const Matrix2d& mat)
{
...
void printMatrix1D(const Matrix1d& mat)
{
    for (unsigned int i = 0; i < mat.size(); i++)
    {
        std::cout << mat[i] << "  ";
...
Matrix2d transpose(const Matrix2d &mat)
{
...
Matrix1d sumMatrix(const Matrix2d& mat, const int axis)
{
    Matrix1d sum;
    if (axis == 0)
    {
...
Matrix1d argmax(const Matrix2d& mat)
{
...
Matrix2d read_mnist_imgs(const std::string& path)
{
    Matrix2d dataset;
...
Matrix2d read_mnist_labels(const std::string& path)
{
    Matrix2d dataset;
    std::ifstream file(path, std::ios::binary);
    if (file.is_open())
...
void batch_data(const Matrix2d& data, Matrix2d& batched_data, const int STEP)
{
...
void ProgressBar(const int step, const int total)
{
...
double avg(const double array[], const int size)
{
    double avg = 0.0;
    double sum = 0.0;
